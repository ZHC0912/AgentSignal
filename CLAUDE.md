...
> This section is the live state of the project. The numbered spec below (§0–§14) is the original brief and stays stable; this section tracks what's actually built and how to pick the work up on any machine. Last updated **2026-07-04**. For a quick cross-machine resume, see **[`HANDOFF.md`](HANDOFF.md)**.
...
| 6.5 | **UI layout refinements** (three-piece layout + glow-clip fix + 3× size cap + dot spacing) | ✅ **accepted (2026-07-01)** — horizontal refinements confirmed live; dot spacing tightened (`DotsView` `StackPanel Spacing="-14"`). See A.7. |
| 6.6 | **Orientation + smart pill direction + collapsible timer** (+ click rewire) | 🟦 **built, headless-verified (2026-07-01)** — layout matrix renders all 6 cases, both orientations launch clean; **awaiting live human eyeball**. See A.7. |
| 6.6-fixes | **Per-session timers (FIX 1) + direction-aware chevron (FIX 3) + dots-anchor stability (FIX 4) + stale-yellow demotion (FIX 2)** | 🟦 FIX 1/3/4 **built & headless-verified**, still await their live eyeball. FIX 2's 30s display demotion was built, owner-confirmed, then **REVERSED and removed 2026-07-04** (it showed false green during long no-hook thinking; see the new Decision #3 in A.2) — the stuck-yellow answer is now the **manual reset** (6.7). |
| 6.6-ui | **Pill-width swap** (timer pill content-sized and the wider one; gear + collapsed-chevron pills = snug 26×26 icon squares) | 🟦 **built, headless-verified (2026-07-03)** — `--anchor-test` still ALL PASS; **awaiting live eyeball**. See A.8. |
| 6.7 | **Manual reset (Settings button + global Ctrl+Alt+R) + modal Settings** | 🟦 **built (2026-07-04)** — `--reset-test` ALL PASS + the **hotkey proven live end-to-end** (injected Ctrl+Alt+R rewrote a planted stuck-yellow file to green/ManualReset on the running widget); **modal shield confirmed working live by the owner** (clicks on the inert widget are silently ignored — the flash/ding cue did nothing on the owner's machine and is **removed/parked**); the reset UX on a real stuck yellow still awaits its eyeball. See A.9. |
| 7 | Polish + Windows/Linux packaging | 🟦 **Windows half built (2026-07-04)** — self-contained single-file publishes (App ≈46MB with icon, Writer ≈11MB trimmed; both verified running with NO .NET available — stripped `DOTNET_ROOT`/PATH), traffic-light `app.ico` (generated, multi-size), `Tmds.DBus.Protocol` pinned **0.21.3** (NU1903 cleared — 0.22–0.91 are vulnerable again, patched only at 0.92; stay on 0.21.x for Avalonia 11.2), `StartupManager` registers the exe itself when self-contained (no dll beside a non-dotnet host process), one-step **`install.ps1`** → `%LOCALAPPDATA%\AgentSignal` (verified live: installed, hooks merged, widget launched from the stable path), portfolio **README** + fresh `docs/` renders. **v1.0.0 tag/Release staged (zip + exe + notes + SHA256s in the session scratchpad), awaiting explicit owner go-ahead**; `gh` is authenticated so it can be created on command. Linux packaging still to do. |
...
3. **The light only changes on REAL events — no display-side guessing. (REVERSED 2026-07-04; supersedes the original Decision #3 stale-yellow→green auto-demotion.)** *History:* because no Claude hook fires on an Esc-abort (`Stop` skips user interrupts, feature request claude-code#9516; silent tool stop #29881 — confirmed live 2026-07-03 with a zero-event capture), a 30s stale-yellow → green **display demotion** was built (`Services/StaleYellow.cs`, `--demote-test`). *Why it was removed:* its accepted gap (b) — a long no-hook thinking/deliberation stretch — turned out to be common and bad in practice: the owner watched the widget go **green at 1m36s while Claude was visibly still "Deliberating"**. Thinking fires no hooks, so **no threshold can distinguish "still thinking" from "aborted"** — the demotion was structurally a guess, and a false "done" is worse than a stale "working". *The decision:* **yellow while working, green only on a real `Stop`** (or a liveness `off`). A yellow stuck by an Esc-abort is cleared **manually** via the reset feature (Ctrl+Alt+R / the Settings Reset button, A.9) — which exists precisely for this and forces the files green through the normal contract. `StaleYellow.cs`, `WidgetViewModel.AggregateDisplayed`, `SessionRowViewModel.IsDemoted` and `--demote-test` are all deleted; `QuietGreen` remains (now only for the reset's quiet green). **Do not reintroduce an inactivity timeout for the light** — this is the same rule as §13's "no inactivity timeout" for liveness, and it's now owner-ratified twice.
...
- `src/AgentSignal.App/` — Avalonia widget.
  - `Services/`: `WorkTimer` (pure per-session stopwatch), `SessionResetService` (manual reset: rewrites every non-green session file to green + `event:"ManualReset"` through the normal state contract; dir-parameterised for `--reset-test`), `GlobalHotkey` (`IGlobalHotkey`; Windows = `RegisterHotKey` Ctrl+Alt+R on the widget HWND + `Win32Properties.AddWndProcHookCallback` for `WM_HOTKEY`; Noop elsewhere), `SessionReader`, `ConfigService` (singleton: `Instance`, `Current`, `Update(mutate)`→save+`Changed` event), `ThemeService` (writes colour brushes + glow `BoxShadows` into `Application.Resources` as **DynamicResource**), `Alerts` (`ISoundPlayer`=`SystemSoundPlayer` Console.Beep on bg thread + `INotifier`=`ToastNotifier`→`ToastWindow`; `AlertService.OnRed/OnGreen/Test`), `StartupManager` (`IStartupManager`; Windows = `reg.exe` HKCU `…\Run` value `AgentSignal`; Noop elsewhere).
  - `ViewModels/`: `DotsViewModel` (abstract base — state + timer text + active flags + static `FormatElapsed`; `DotsOrientation` (bound by `DotsView`); also owns the **reusable green-blink**: `IsGreenPulsing`, started in `OnStateChanged` on entering green for `config.BlinkOnGreenSeconds`, cancelled at once on any other transition, expired by `TickPulse(now)` from the poll), `SessionRowViewModel : DotsViewModel` (one per session, owns a `WorkTimer`), `WidgetViewModel : DotsViewModel` (reconciles `ObservableCollection<SessionRowViewModel>` **in place** each 250ms tick — no `Clear()` so no flicker — picks the driving session; fires **per-session red alerts, debounced** via a `_redAlerted` set, plus the aggregate green edge; also owns the **layout** the view binds to — `IsVertical`→`AttachDock`/`AttachMargin`/alignments/`SessionsStackOrientation`, `AttachFlip` (smart direction), `IsGearVisible` (dot-click reveal), `IsExpanded` (**automatic**: `SessionCount>1`), `IsTimerCollapsed` (persisted)), `SettingsViewModel`.
  - `Views/`: `WidgetWindow` (+ `LayoutTransformControl` `ScaleHost` for live scale/opacity), `PillView`, `DotsView` (the `.dot.pulse` opacity animation = the green-blink, bound to `IsGreenPulsing`), `SettingsWindow`, `ToastWindow`.
    - **Layout (A.7, as of 6.6):** `PillView` root is a **`DockPanel`**: the dots block fills, and an **attach strip** (`Grid`) holding the **timer** (or its **chevron** when collapsed) at one end and the **gear** at the other docks to whichever edge `AttachDock` picks — **below/above** the dots when horizontal, **right/left** when vertical (smart direction). The strip stretches across the dots' cross axis; **horizontal** pins timer left / gear right (opposite corners), while **vertical** hugs both pills against the dots-facing edge of the strip (`TimerHorizontal`/`GearHorizontal` = Left, or Right when flipped) with the gear stacked **directly below** the timer/chevron (`GearMargin` top = slot 26 + 4) so they read as one tight cluster beside the column instead of floating in the 84px reservation (owner's spacing call, 2026-07-02). **Collapsed (0–1 session)** = single aggregate dots pill + the driving session's timer in the strip; **expanded (auto, 2+ sessions)** = one **row per session** in an `ItemsControl` (panel stacks on `SessionsStackOrientation`), and **each row carries its OWN independent timer** (from that session's `WorkTimer`) beside its dots (row = `StackPanel Orientation="{Binding DotsOrientation}"`, `Spacing=7` dots↔timer, `Spacing=12` between rows). The single strip timer/chevron **hides while expanded** (`IsTimerShown`/`IsTimerChevronShown` gate on `!IsExpanded`) so there's no redundant driver-hopping number. Timer/chevron/gear are `Button.flat` (each **consumes its own click**); the gear/chevron use `Button.glyph` (icon font). The collapse **chevron is direction-aware** (`TimerChevronGlyph`): `‹` when the strip opens leftward (`AttachDock==Left`, i.e. vertical + flip), else `›`. Timer text is a plain `TextBlock` — **never rotated**, so it stays readable in both orientations. **The dots pill is the fixed anchor:** the timer + chevron live inside a **fixed-size transparent reservation slot** (`Panel Width=84 Height=26`, pre-scale, shown while `!IsExpanded`) so swapping chip⇄chevron, varying timer-text width, or hiding the timer **never changes the strip's footprint** and so **never nudges the dots** in any orientation/flip. The collapsed **chevron appears at the timer's own spot** (owner's live-eyeball call, 2026-07-02 — it briefly centred on the dots instead, then was moved back): it sits **inside the reservation slot with the same alignment as the timer chip** (`TimerHorizontal`/Center), so chip ⇄ chevron simply **swap in place** — left under the dots when horizontal, at the strip's top when vertical. The chevron pill hugs its glyph like the gear: it renders `‹`/`›` in a **normal UI font** (`Segoe UI, Inter`), NOT the `Segoe Fluent Icons` glyph font (which drew a wide fallback for those codepoints — see A.6). Proven by `--anchor-test` (dots offset identical across chip/chevron/wide-text/hidden in all 4 docks).
    - **`DotsView`** lays the three dots on `{Binding DotsOrientation}` (row or column), each centred in a **44×44 transparent `Border.cell`** with `Spacing="-14"` between cells (tightened). The cell is load-bearing: Avalonia clips a control's `BoxShadow` to its arrange slot, so a bare 15×15 dot cut the round glow into a **square**; the roomy cell lets the glow fade fully → round halo (fix documented in A.6). Dots-pill padding is therefore small (`8,2`).
    - **`WidgetWindow` clean-click = reveal the gear** (`IsGearVisible`), no longer expand (expansion is automatic). On drag-release / startup / orientation change it runs `SettleLayout()`: sets `AttachFlip` from the window's screen rect (near far edge → flip inward, `EdgeMargin` 56 DIPs) then **clamps fully on-screen**; orientation changes settle via a one-shot `LayoutUpdated` (size changes first).
  - `Models/AppConfig`: `Orientation` ("Horizontal"|"Vertical", default Horizontal), `TimerCollapsed` (bool, persisted); `BlinkOnGreenSeconds` (0–5, default 2; 0 = off); `Scale` range is **0.6–3.0** (settings slider cap raised from 2× to 3×; default still 1.0).
...
> **Phase 7 stable install (preferred for daily use):** `install.ps1` (repo root; also shipped in the release zip) copies the self-contained `AgentSignal.exe` + `AgentSignal.Writer.exe` to **`%LOCALAPPDATA%\AgentSignal`**, runs the hook install, and (with `-Startup`) registers launch-on-startup pointing at that stable exe — on an installed copy neither the `DOTNET_ROOT` quirk nor the stale-`bin\Release` caveat below applies (`StartupManager` registers the running exe itself when it's self-contained). Publish flags for both exes are in the README's build-from-source section.

> **Launch-on-startup is per-machine and bakes in an absolute build path.** The HKCU `Run` value is written by `StartupManager.LaunchCommand()` from **`AppContext.BaseDirectory` of whichever process registered it** — i.e. `"~\.dotnet\dotnet.exe" "<that machine's repo>\src\AgentSignal.App\bin\Release\net10.0\AgentSignal.App.dll"`. It never updates itself, so: (a) it must be (re-)registered on each machine (Settings toggle or `--startup on` **run from the Release build**); (b) it serves whatever is in `bin\Release` at logon — rebuilding Release in place keeps it current, but a silently-failed build leaves the old dll (this caused two "old UI at boot" incidents). Phase 7's self-contained publish to a stable install path is the real cure.
...
- `--screenshot <path>` — render a static preview PNG (state sheet + a layout matrix at the bottom).
- `--layout <path>` — render just the **orientation × smart-direction × collapsible-timer** matrix (6 cases) in a grid, so every case is fully visible (the full `--screenshot` sheet exceeds one frame).
- `--anchor-test` — headless pixel-stability proof: for all 4 attach docks, measures the dots pill's offset while toggling the timer chip⇄chevron, widening the timer text, hiding the timer, **and toggling the gear on/off**; **PASS = dots never move** (exit 0). Proves the fixed reservation-slot anchor (gear-off is where a chevron height-leak hides).
...
- `--reset-test` — prove the manual reset (A.9): `ForceGreen` against a **temp** sessions dir rewrites yellow AND red files to green/`ManualReset` (pid preserved, green files untouched), and a real `SessionRowViewModel` observing the forced green freezes its timer as the last run's time with no blink, then starts the next run fresh from 0. Exit 0 = ALL PASS.
...
- **The icon font (`Segoe Fluent Icons`) renders a WIDE fallback for non-icon codepoints — and headless can't see it.** The gear `⚙` (U+2699) is a real icon in that font, but the chevron `‹`/`›` (U+2039/U+203A) are ordinary punctuation with **no glyph** there, so on live Windows the font drew a ~62px-advance fallback → the chevron pill ballooned to ~84px while the gear stayed small. Headless has no `Segoe Fluent Icons` (falls back to Inter), so `--screenshot`/`--layout` rendered the chevron *narrow* and **hid the bug** — a live-only font issue the headless renders will not reproduce. Fix: render `‹`/`›` in a **normal UI font** (`Segoe UI, Inter`), reserving the icon font for actual icons (the gear). When a "glyph" looks wrong only on the live widget, suspect the icon font lacks that codepoint.
- **A control that sits OUTSIDE the fixed reservation slot leaks its size into the strip.** When the collapsed chevron was briefly a direct strip child (to centre it on the dots), its **height** fed the strip height in the horizontal-flip (Top) dock and pushed the dots down when it exceeded the slot. The chevron has since moved **back inside the slot** (owner wanted it at the timer's spot, 2026-07-02), which removes the leak by construction — but the lesson stands: anything added to the strip outside the slot must fit within the slot's reserved 26px height (which covers the tallest element, the gear). The `--anchor-test` toggles the **gear on AND off** — the gear is hidden by default, which is exactly where this class of leak hides.
...
### A.7 Phase 6 & 6.5 (accepted) + Phase 6.6 orientation/direction/timer (awaiting live eyeball)
...
**Phase 6.5 — UI layout refinements (ACCEPTED, confirmed live 2026-07-01).** Reworked the widget from one combined pill into separate chips, fixed the dot glow, raised the size cap, and tightened dot spacing. The three-piece layout, round glow, alignment, tighter gap and 3× crispness were all confirmed on-screen by the owner.

- ✅ **Three-piece chips**, **per-dot square glow-clip fixed** (44×44 `Border.cell` → round halo, see A.6), **gear matched to timer** (shared style), **timer pinned left / gear pinned right** aligned to the dots-pill edges, **size cap 2×→3×**. *Accepted live.*
- ✅ **Dot spacing tightened** — `DotsView` `StackPanel Spacing="-14"` pulls the three dots closer (center-to-center 44→30px) without shrinking the 44px glow cell. *Accepted live.*

**Phase 6.6 — Orientation + smart pill direction + collapsible timer + click rewire (built, headless-verified 2026-07-01; awaiting a live human eyeball).** Three connected changes plus a clean click rewire, all sharing the new `DockPanel` layout. Verified via `--layout` (all 6 cases render correctly) and a clean launch in **both** orientations; the timer regression still yields 1:10 and config stays backward-compatible (new fields default). Not yet eyeballed live.

- ✅ **Vertical orientation (manual setting).** `AppConfig.Orientation` (Settings → APPEARANCE combo, persisted, applied live). Vertical stacks the three dots in a **column** with the timer/gear pills **beside** it (not below); the timer text stays **horizontal/readable** in both. On an orientation change the window re-clamps fully on-screen (a tall vertical widget near a corner can't hang off).
- ✅ **Smart pill direction (auto, Behaviour B).** The window computes near-edge from its screen rect and sets `AttachFlip`, so the attached pills open **toward** the screen: horizontal flips **below→above** near the bottom edge; vertical flips **right→left** near the right edge. Triggered within a margin (`EdgeMargin` 56 DIPs), not only when jammed against the edge, and a final clamp guarantees nothing clips.
- ✅ **Collapsible timer (own control).** Click the timer → it hides and a **chevron** (`›`) takes its slot; click the chevron → it returns. The `WorkTimer` **keeps counting** while collapsed (visual only), and the state persists across relaunch (`AppConfig.TimerCollapsed`). The chevron takes the timer's position; the gear keeps its own.
- ✅ **Click behaviour rewired.** Clicking the **dots** now reveals the **gear only** (`IsGearVisible`) — it no longer toggles the multi-session view. The per-session rows appear **automatically** when 2+ sessions are live (`IsExpanded = SessionCount>1`) and collapse back to the aggregate at one. Timer/chevron/gear are each `Button.flat` so they **consume their own click** (the proven gear pattern) and never trigger each other or the dots-click.
- 🟦 **Still to do (6.6):** live human eyeball of orientation switching, the edge-flip while dragging near a screen edge, and the timer collapse/expand on the running widget. Also still carried over: the **green-blink** pulse animation has never been eyeballed live.
...
### A.8 Phase 6.6 follow-up fixes (2026-07-01)

Fixes requested against the 6.6 build (all four built & headless-verified; FIX 2 resolved 2026-07-03). **Do NOT start Phase 7 until the owner has eyeballed everything live.**

- ✅ **FIX 1 — per-session timers in the multi-session view (built, headless-verified).** Before, the expanded view showed one dots pill per session but a **single shared timer** (the attach-strip readout bound to `DrivingTimerText` = red-wins-else-most-recent-`ts`), which visibly **jumped between sessions** as their `ts` updated. Now **each expanded row shows its OWN independent timer** from that session's `WorkTimer` (already existed on `SessionRowViewModel`; just surfaced in the `ItemsControl` `DataTemplate` beside its dots — `Spacing=7` dots↔timer, row `Spacing=12`). The single strip timer/chevron **hides while expanded** (`IsTimerShown`/`IsTimerChevronShown` now also gate on `!IsExpanded`). Collapsed single pill still shows just the driving session's timer. Verified via `--screenshot` (two rows, `0:48` red-lit + `2:03` yellow-lit, each its own chip; also at 3×). Each row's `WorkTimer` is independent, so no row's time changes when another ticks. **Live-verify with two real `claude` sessions.**
- ✅ **FIX 3 — direction-aware collapse chevron (built, headless-verified).** The collapsed-timer chevron was always `›`. Now `WidgetViewModel.TimerChevronGlyph` returns **`‹` when `AttachDock==Left`** (vertical widget flipped left near the right edge, so the timer would reappear leftward) and **`›`** otherwise (horizontal below/above, vertical-right — all unchanged). Bound to the chevron `Button.Content`; recomputed off `IsVertical`/`AttachFlip`. Verified in the `--layout` matrix (vertical-right-collapsed = `›`, vertical-near-right+collapsed = `‹`).
- ✅ **FIX 4 — dots pill is now a fixed anchor; timer/chevron can't nudge it (built, headless-verified).** Problem: toggling the timer chip ⇄ collapse chevron (or a different timer-text width, or a chip/chevron height difference from text-12 vs glyph-13) changed the attach strip's footprint and **pushed the dots pill sideways/downward** — worst in the flip cases where the strip docks on the *near* side of the dots (`Dock.Left`/`Dock.Top`). Fix: the timer chip and chevron now live inside a **fixed-size transparent reservation slot** (`Panel Width=84 Height=26`, pre-scale, `IsVisible="{Binding !IsExpanded}"`; height 26 = the gear's height, the tallest strip element); only the element *inside* swaps, so the strip footprint is constant and the dots never reflow — in **all** orientations and `AttachFlip` directions, and whether the timer is shown or hidden. Proven by the new **`--anchor-test`** (all 4 docks PASS: dots offset identical across chip/chevron/`12:34:56`/hidden — horizontal `(9,3)`/`(9,32)`, vertical `(9,3)`/`(96,3)`), and the `--layout` matrix still renders correctly. **Live-verify: toggle the timer on the running widget in each orientation/edge — dots shouldn't budge.**
- 🟦 **Pill-width swap (2026-07-03, owner-directed; built, headless-verified, awaiting live eyeball).** The sizing pair is now: **timer pill = content-sized** (the wider one — `Padding 11,3`, `MinHeight 22`; anything from `0:09` to `12:34:56` fits without clipping, and the ~82px worst case still fits the fixed 84px reservation slot so the dots anchor holds); **gear + collapsed-chevron pills = snug 26×26 icon squares** (`Padding 0`, glyph centred by the flat Button; 26 = the slot height, so the vertical GearMargin cluster math is unchanged, and the fixed width also hard-caps the A.6 icon-font wide-fallback hazard). This **supersedes** the interim owner experiments (gear `Width=60`; timer `36×26`). All in the `Border.pill.timer` / `Border.pill.gear` styles in `PillView.axaml` — the `Border` is the visible pill and the element to size. `--anchor-test` ALL PASS, baselines unchanged.
- ⛔ **FIX 2 — Esc-cancel stuck-yellow: first resolved via a 30s stale-yellow display demotion (2026-07-03, owner-confirmed), then REVERSED and removed 2026-07-04.** The investigation stands and remains the ground truth: the owner reproduced a real Esc-abort with full-event logging hooks live — the aborted session's trail was `SessionStart → UserPromptSubmit → PreToolUse → nothing`. **Zero events fire on an Esc-abort** (the Esc even swallowed the `PostToolUse` the just-finished tool was owed), matching the docs (`Stop` does not run on user interrupt; an interrupt hook is only feature request #9516; silent tool stop #29881 also fires nothing); the `Notification`/`idle_prompt` event was chased and ruled out. The demotion built on that was removed because it showed **false green during long no-hook thinking** (see the reversed Decision #3 in A.2 for the full why). The stuck-yellow answer is now the **manual reset (A.9)**; the temporary logging hooks stayed removed throughout.

### A.9 Manual reset + modal Settings (2026-07-04, owner-directed)

Two features on top of 6.6. Built & regression-clean (`--reset-test` 10/10 + all prior proofs still pass); the hotkey path was **verified live end-to-end** (OS-injected Ctrl+Alt+R → the running widget rewrote a planted stuck-yellow session to green); the **modal shield is owner-confirmed working live**; the reset UX on a real stuck yellow still awaits its eyeball.

- 🟦 **Feature A — Manual reset (THE mechanism for clearing any Esc-abort stuck-yellow, since the auto-demotion was reversed — Decision #3).** Forces ALL current sessions to green *as if the turn completed*: `SessionResetService.ForceGreen` **rewrites the session files** (state=green, `event:"ManualReset"`, ts=now, pid preserved — same JSON contract as the writer), so the normal machinery does the rest: each `WorkTimer` sees yellow→green and **freezes at its current value** (shown as the last run's time), and the next `UserPromptSubmit` starts a fresh yellow+timer. Yellow AND red both clear (a red whose prompt is really still live self-corrects on the next hook event). **Quiet by design:** no green alert and no green-blink — `WidgetViewModel.ResetAllSessions()` sets a one-shot `_quietReset` (suppresses the aggregate alert edge + blink on the immediate refresh), and each row keys `QuietGreen` off `event=="ManualReset"`. Triggers: **(1)** the RESET section's button in Settings (`SettingsViewModel.ResetSessionsCommand` → an `Action` injected by `App`); **(2)** **global Ctrl+Alt+R** (`GlobalHotkey`, Windows `RegisterHotKey` + `MOD_NOREPEAT`, `WM_HOTKEY` picked off the widget window's WndProc via `Win32Properties.AddWndProcHookCallback`; Noop on other OSes until Phase 7+). The hotkey is **unregistered while Settings is open** and re-registered on close (the in-window button covers that case). Proven by `--reset-test`; hotkey confirmed live. *Live-verify remaining:* Esc mid-tool → stuck yellow → both the button and a hand-pressed Ctrl+Alt+R force green with the timer frozen.
- 🟦 **Feature B — Modal Settings (widget inert while Settings is open).** `App` raises `SettingsOpenChanged(bool)` around the Settings window's lifetime. The widget shows `ModalShield` — a transparent but hit-testable `Border` overlaying the whole window — which **swallows every press** (no click, no drag, no gear/timer buttons; expansion was already automatic). **Clicks on the inert widget are SILENTLY ignored — there is deliberately NO attention cue** (owner ruling, 2026-07-04; **confirmed working live**): a flash+ding nag (`FlashWindowEx` `FLASHW_ALL|FLASHW_TIMERNOFG` + `MessageBeep`, three variants — fixed-count, flash+`Activate()`, decoupled standalone beep+flash with a debug trace) **produced nothing visible/audible on the owner's machine and was removed; the cue is PARKED — do not reattempt it unless the owner asks.** `SettingsWindow` keeps `ShowInTaskbar="True"` and opens unowned via `Show()` (a normal app dialog with its own taskbar button). A defence-in-depth guard in `WidgetWindow.OnPointerPressed` also bails while `App.IsSettingsOpen`. On close the shield hides and Ctrl+Alt+R re-registers. Only the widget is affected; the rest of the OS is normal. *Live-verify:* open Settings → widget won't drag/click (clicks do nothing), Ctrl+Alt+R dead until Settings closes, then everything returns.
