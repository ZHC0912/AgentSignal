...
- `src/AgentSignal.App/` — Avalonia widget.
  ...
  - `Views/`: `WidgetWindow` (+ `LayoutTransformControl` `ScaleHost` for live scale/opacity), `PillView`, `DotsView` (the `.dot.pulse` opacity animation = the green-blink, bound to `IsGreenPulsing`), `SettingsWindow`, `ToastWindow`.
    - **Layout (A.7, as of 6.6):** `PillView` root is a **`DockPanel`**: the dots block fills, and an **attach strip** (`Grid`) holding the **timer** (or its **chevron** when collapsed) at one end and the **gear** at the other docks to whichever edge `AttachDock` picks — **below/above** the dots when horizontal, **right/left** when vertical (smart direction). The strip stretches across the dots' cross axis so timer/gear pin to opposite corners. Collapsed = single aggregate dots pill; expanded (auto, 2+ sessions) = one dots pill per session in an `ItemsControl` whose panel stacks on `SessionsStackOrientation`. Timer/chevron/gear are `Button.flat` (each **consumes its own click**); the gear/chevron use `Button.glyph` (icon font). Timer text is a plain `TextBlock` — **never rotated**, so it stays readable in both orientations.
