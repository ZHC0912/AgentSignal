...
| 6.6-fixes | **Per-session timers (FIX 1) + direction-aware chevron (FIX 3) + dots-anchor stability (FIX 4) + stale-yellow demotion (FIX 2)** | 🟦 **all four built & headless-verified**; FIX 2 resolved 2026-07-03: the event capture proved Esc fires NO hook, so the 30s display-only demotion was built (locked Decision #3, `--demote-test` ALL PASS), temp logging hooks removed from `~/.claude/settings.json`, and the **owner confirmed FIX 2 working live**. FIX 1/3/4 still await their live eyeball. |
...
| 7 | Polish + Windows/Linux packaging | ⏳ **next** (gated on the live eyeballs + explicit owner go-ahead) — a self-contained App build would remove the `DOTNET_ROOT`-at-logon caveat (see A.4); also bump `Tmds.DBus.Protocol` (NU1903 advisory) then |
...
3. **Stale-yellow → green display demotion (the Esc-abort/silent-stop fallback, resolves FIX 2).** *Why:* no Claude hook fires on a user interrupt — `Stop` explicitly does NOT run on Esc (an interrupt hook is only an open feature request, claude-code#9516), and a tool can also end a turn silently with no `Stop` (#29881). Confirmed live 2026-07-03: a full-event capture of a real Esc-abort logged **zero** events after the abort. *The fix:* a session that has sat `yellow` with **no file update for 30 s (hardcoded, no setting)** is **displayed** green. It is **presentation-only** — the `WorkTimer` and state data are never touched (the timer keeps counting underneath), so if a later event (e.g. `PostToolUse`) proves the session was actually still working, the display snaps back to yellow with the correct timer value intact, never reset. **In-flight-tool guard:** a session whose last event is `PreToolUse` (no matching `PostToolUse` has overwritten it) is **NEVER demoted** — long installs/builds stay yellow. **No green alert and no green-blink on a demotion** (it's a guess, not a confirmed finish); alerts key on the *real* aggregate, so the real finish after a false demotion still fires its green alert. §13 liveness (green→off) untouched — only yellow demotes, red never does. *Accepted gaps:* (a) an Esc that lands mid-tool swallows the pending `PostToolUse`, so its file is byte-identical to a running tool's and stays yellow (the guard is deliberately conservative); (b) a >30 s no-tool thinking/streaming stretch shows a false green until the next event, then snaps back. Implemented in `Services/StaleYellow.cs` (the rule) + `SessionRowViewModel.Observe` (per-row) + `WidgetViewModel.AggregateDisplayed` (aggregate) + `DotsViewModel.QuietGreen` (blink suppression); proven by `--demote-test`.
...
- `--demote-test` — prove the stale-yellow→green display demotion (Decision #3) with a controlled clock through the real `SessionRowViewModel`/`WorkTimer`: demotes at 30s with the timer still counting underneath and no blink, snaps back to yellow with the timing intact on a later event, and never demotes an in-flight `PreToolUse` or a red. Exit 0 = ALL PASS. (`--watch` rows also show `stale-yellow→green` while a session is demoted.)
