...
- **Know which session is which** — each pill is labelled with its project folder and agent (`folder · Tool`), shown as initials and expanding to the full name on hover.
- **Hide what you don't need** — right-click to hide a single session's pill; Settings → Session Tracking lists every session with a show/hide checkbox.
...
The hooks only ever report a state word (`green` / `yellow` / `red` / `off`); all timing logic lives in the widget, derived from state transitions. Session liveness is real: a session disappears when it ends *or* its process dies (PID captured at session start) — never on an inactivity timeout, so a healthy idle agent stays green forever. A session is also matched to its process by start time, so a recycled process ID can't bring a dead session back, and dead session files are deleted rather than left behind.
